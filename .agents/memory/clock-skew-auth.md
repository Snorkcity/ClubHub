...
**How to apply:** Dev server tolerates drift via `clockSkewInMs` (30 min, dev-only) in the clerkMiddleware options in the api-server app setup. Production stays strict. Temp debug logging in requireAuth's 401 branch decodes token iss/exp — remove it once auth debugging is done.
