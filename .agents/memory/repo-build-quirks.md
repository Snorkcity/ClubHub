...
**No `tsx` at the repo root** (`pnpm exec tsx` fails there), but the `scripts/` workspace has it — put standalone TS scripts in `scripts/src` and add a package script. The api-server runs via esbuild (`build.mjs`), not a TS loader. To run a standalone TS script (e.g. a DB seed):
